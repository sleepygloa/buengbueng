...
<tr>
		...
		<td class="tt2"><a href="customerContent.do?num=${list.num}&snum=${snum}&pageNum=${pageNum}&number=${number}&find=1">${list.title}</a></td>
...
<div class="footer">  	
	<c:if test="${startPage > 10}">
        <a href="serviceCenter.do?snum=${snum}&pageNum=${ startPage - 10 }&find=1">[이전]</a>
	</c:if>
	<c:forEach var="i" begin="${startPage}" end="${endPage}">
		<a href="serviceCenter.do?snum=${snum}&pageNum=${i}&find=1">[${i}]</a>
	</c:forEach>
	<c:if test="${endPage < pageCount}">
    	<a href="serviceCenter.do?snum=${snum}&pageNum=${ startPage + 10 }&find=1">[다음]</a>
	</c:if>
</c:if>
</div>
</div>
