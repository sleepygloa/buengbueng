	...
	<input type="hidden" id="l_key" value="${l_key}" />
    	...
    	<c:forEach var="ol" items="${orderList}">
    		...
    		<td>${ol.num}</td><td>${ol.id}</td><td>${ol.menuname}</td><td>${ol.ordertime}</td><td>${ol.ordermoney}</td>
    			<c:if test="${ol.orderstatus==1}"><td style="background-color:#FFA7A7">주문 중</td>
    			<td>
    			...
    			<c:if test="${ol.orderstatus==2}"><td style="background-color:#B2CCFF">주문완료</td><td></td></c:if>		
    			<c:if test="${ol.orderstatus==3}"><td background="#B2CCFF">주문취소</td><td></td></c:if>
    			<c:if test="${ol.orderstatus==4}"><td style="background-color:#FFE08C ">환불 요청 중</td>
    			<td>
    			...
    			<c:if test="${ol.orderstatus==5}"><td style="background-color:#B2CCFF ">환불 승인</td><td></td></c:if>
    			<c:if test="${ol.orderstatus==6}"><td style="background-color:#B2CCFF ">환불 거절</td><td></td></c:if>
